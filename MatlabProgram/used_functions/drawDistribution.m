...
%              plot(a(1+100*(vff-1):100*(vff)) + b(1+100*(vff-1):100*(vff)), '.-g');hold on;
            ...
            fig = visualisationShared(promp{1}.PSI_z*promp{1}.mu_w, promp{1}.PSI_z*1.96*sqrt(diag(promp{1}.sigma_w )), sum(promp{1}.traj.nbInput), z,  vff, 'g', fig);
            fig = visualisation(promp{1}.PSI_z*promp{1}.mu_w, sum(promp{1}.traj.nbInput), z,  vff, 'g', fig);
            set(gca, 'fontsize', 20);
            disG = size(fig,2);
